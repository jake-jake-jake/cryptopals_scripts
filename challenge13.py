...
def ECB_decrypt(cipher, key):
    ...
    ECB_cipher = AES.new(key, AES.MODE_ECB)
    return ECB_cipher.decrypt(cipher)
...
print("insertion_block", insertion_block, len(insertion_block))
...
print("attack_suffix", ECB_decrypt(attack_suffix,static_key), len(attack_suffix))
attack_prefix = cookie_oracle((switch_email), static_key)[:32]
print("attack_prefix", ECB_decrypt(attack_prefix, static_key))
